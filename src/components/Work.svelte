...
<li class="mb-10 ml-6">
    ...
    <h3 class="flex items-center mb-1 text-lg font-semibold text-white">{position}</h3>
    <h3 class="flex items-center mb-1 text-lg font-semibold text-white">
        <span class="text-base font-medium mr-2 px-2.5 py-0.5 rounded-lg ed bg-blue-900 text-blue-300">
            <a href="/work/musicreports">{name}</a>
        </span>
        <span>{location}</span>
    </h3>
    <time class="block mb-2 text-base">{date}</time>
</li>